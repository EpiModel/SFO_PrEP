...
# set the compression method for `saveRDS`
comp_method <- FALSE
...
if (! file.exists("out/est/epistats.rds")) {
  epistats <- build_epistats(
    ...
    geog.cat = "San Francisco",     # default is NULL
...
# Netparams build
if (! file.exists("out/est/netparams.rds")) {
  netparams <- build_netparams(
    epistats,
    smooth.main.dur = TRUE # default is FALSE
  )
  saveRDS(netparams, "out/est/netparams.rds", compress = comp_method)
} else {
